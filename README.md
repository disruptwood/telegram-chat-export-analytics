...
The project is designed around raw Telegram export folders and focuses on reproducible analytics rather than notebook-only exploration. It currently supports message-volume trends and participant activity over time, and the structure is meant to stay easy to extend with new metrics.

## Principles

- One source of truth for Telegram export loading: `chat_analytics.loader`.
- Domain model, aggregations, reporting, and CLI are separated.
...
Documentation for these files and metrics: `docs/USER_ACTIVITY_DATA.md`.
